...
function defaultState(){
  return {transactions:[],categories:[...DEFAULT_CATEGORIES],accounts:[...DEFAULT_ACCOUNTS],
          income:[],incomeCategories:[...DEFAULT_INCOME_CATEGORIES],transfers:[],budgets:{}};
}
function loadLocal(){
  try {
    const r = localStorage.getItem('et_v4');
    if(r){ const s=JSON.parse(r); ['transactions','income','transfers'].forEach(k=>{if(!s[k])s[k]=[];}); if(!s.categories)s.categories=[...DEFAULT_CATEGORIES]; if(!s.accounts)s.accounts=[...DEFAULT_ACCOUNTS]; if(!s.incomeCategories)s.incomeCategories=[...DEFAULT_INCOME_CATEGORIES]; if(!s.budgets)s.budgets={}; return s; }
  } catch(e){}
  return defaultState();
}
let st = loadLocal();
