...
function authErr(e) {
  ...
  const map = {
    ...
    "auth/configuration-not-found":
      "Firebase project not configured correctly. Check your config in js/config.js.",
    "auth/app-not-authorized":
      "App not authorised. Check Firebase project settings.",
...
async function doLogin() {
  ...
  if (!window._fbAuth) {
    errEl.textContent = "Firebase not initialised. Check your config.js.";
    return;
  }
  setAuthBtn("a-login-btn", true, "Sign In");
  try {
    await window._fbAuth.signInWithEmailAndPassword(
      window._fbAuth.auth,
      email,
      pass,
    );
...
async function doSignup() {
  ...
  try {
    const cred = await window._fbAuth.createUserWithEmailAndPassword(
      window._fbAuth.auth,
      email,
      pass,
    );
...
async function doPasswordReset() {
  ...
  if (!window._fbAuth) {
    errEl.textContent = "Firebase not initialised.";
    return;
  }
  try {
...
async function initFirebase() {
  ...
  try {
    ...
    onAuthStateChanged(auth, async function (user) {
      currentUser = user;
      if (user) {
        // Use get() for one-time read — avoids re-firing on every save
        const { get } =
          await import("https://www.gstatic.com/firebasejs/10.12.0/firebase-database.js");
        try {
