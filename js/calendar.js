...
function renderCalTxList() {
  ...
  const dateLabel = new Date(calSelected + "T00:00:00").toLocaleDateString(
    "en-SG",
...
function openCalAddModal(date) {
  // populate selects
  document.getElementById("cal-modal-date-label").textContent = new Date(
    date + "T00:00:00",
  ...
  document.getElementById("cal-inp-cat").innerHTML = (state.categories || [])
    .map((c) => `<option>${c}</option>`)
    .join("");
  document.getElementById("cal-inp-acc").innerHTML = (state.accounts || [])
...
function closeCalAddModal() {
  document.getElementById("cal-add-modal").classList.remove("open");
}
