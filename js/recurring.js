...
function getNextDue(rule) {
  return rule.lastRun
    ? advanceDate(parseDate(rule.lastRun), rule.frequency)
...
function refreshRecSelects(type) {
  ...
  const cats =
    (type || recCurrentType) === "income"
      ? st.incomeCategories && st.incomeCategories.length
        ? st.incomeCategories
      ...
      : st.categories && st.categories.length
        ...
        : DEFAULT_CATEGORIES;
  const accs =
    st.accounts && st.accounts.length ? st.accounts : DEFAULT_ACCOUNTS;
  catSel.innerHTML = cats
    .map((c) => '<option value="' + c + '">' + c + "</option>")
    .join("");
  accSel.innerHTML = accs
...
function renderRecurring() {
  ...
  if (!st.recurring.length) {
    list.innerHTML =
      '<div class="empty-state">No recurring rules yet.<br>Add one above to get started.</div>';
    return;
  }

  list.innerHTML = st.recurring
    .map((rule) => {
      const isInc = rule.type === "income";
      const color = isInc
        ? getIncColor(rule.category)
        : getCatColor(rule.category);
      const emoji = isInc
        ? getIncEmoji(rule.category)
        : EMOJIS[rule.category] || "💳";
      const freqLbl =
        {
          daily: "Daily",
          weekly: "Weekly",
      ...
      return (
        ...
        "22;color:" +
        color +
        ';border-radius:99px;padding:2px 7px;font-weight:600;">' +
        typeLbl +
        "</span>" +
        "</div>" +
        (rule.description
        ...
        " · " +
        freqLbl +
        ' · Next: <strong style="color:var(--primary)">' +
        nextDue +
        "</strong>" +
        (!rule.active
        ...
        "<button onclick=\"toggleRecurring('" +
        rule.id +
        '\')" class="rec-btn ' +
        (rule.active ? "rec-btn-pause" : "rec-btn-resume") +
        '">' +
        (rule.active ? "⏸ Pause" : "▶ Resume") +
        "</button>" +
        "<button onclick=\"deleteRecurring('" +
        rule.id +
        '\')" class="rec-btn rec-btn-del">🗑 Delete</button>' +
        "</div>" +
        "</div>"
      );
    })
...
function addRecurring() {
  ...
  if (!cat) {
    errEl.textContent = "⚠️ Please select a category";
    return;
  }
  if (!acc) {
    errEl.textContent = "⚠️ Please select an account";
    return;
  }
  if (!start) {
    errEl.textContent = "⚠️ Please set a start date";
    return;
  }
  const rule = {
    id: "r_" + Date.now(),
